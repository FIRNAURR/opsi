...
# ==================================================================
# 1. KONFIGURASI & AUTHENTIKASI ADMIN
# ==================================================================
st.set_page_config(
...
# ==================================================================
# 2. DATA 6 TITIK WISATA (Koordinat Presisi)
# ==================================================================
DIR_LABEL = {
...
INITIAL_SPOTS = [
    {
        ...
        "amenities": {
            "resto": [("Kedai kuliner Rabbit Forest", "T · 90 m"), ("Golden Pine Cafe", "TL · 140 m")],
            "mushola": [("Mushola dekat loket masuk", "B · 60 m")],
            "fasilitas": [("Toilet umum area parkir", "B · 70 m"), ("Pos informasi & P3K", "T · 100 m")],
        },
    ...
    },
    {
        ...
        "amenities": {
            "resto": [("Restoran Sangkuriang", "TG · 130 m"), ("Saung Pengkolan 2", "T · 80 m")],
            "mushola": [("Mushola dekat camping ground", "BD · 150 m")],
            "fasilitas": [("Toilet & kamar bilas", "T · 90 m"), ("Pos keamanan", "BD · 60 m")],
        },
    ...
    },
    {
        ...
        "amenities": {
            "resto": [("Perahu kuliner Sunda", "T · 40 m"), ("Zona jajanan internasional", "TL · 100 m")],
            "mushola": [("Mushola dekat pintu masuk", "B · 90 m")],
            "fasilitas": [("Toilet area playground", "TG · 70 m"), ("Loket & pos informasi", "B · 50 m")],
        },
    ...
    },
    {
        ...
        "amenities": {
            "resto": [("Kuliner Zona Korea & Jepang", "TL · 120 m"), ("Kuliner Zona Afrika", "T · 160 m")],
            "mushola": [("Mushola dekat Zona Indonesia", "B · 80 m")],
            "fasilitas": [("Toilet Zona India", "TG · 100 m"), ("Pos keamanan pintu masuk", "BD · 60 m")],
        },
    ...
    },
    {
        ...
        "amenities": {
            "resto": [("Warung makan area camping", "T · 100 m"), ("Kedai kopi & jajanan", "TG · 70 m")],
            "mushola": [("Mushola sederhana dekat gerbang", "BD · 130 m")],
            "fasilitas": [("Toilet & kamar mandi umum", "T · 90 m"), ("Pos ranger / pengelola", "B · 50 m")],
        },
    ...
    },
    {
        ...
        "amenities": {
            "resto": [("Kedai susu & jajanan khas", "T · 60 m"), ("Warung makan area piknik", "TL · 110 m")],
            "mushola": [("Mushola dekat pintu masuk", "B · 70 m")],
            "fasilitas": [("Toilet umum", "TG · 80 m"), ("Pos informasi wahana", "B · 40 m")],
        },
...
# ==================================================================
# 3. STATE MANAGEMENT & PARSING LOKASI (GPS)
# ==================================================================
def _init_session_state():
    ...
    st.session_state.setdefault("trigger_locate", False)
    if "spots" not in st.session_state:
        st.session_state.spots = copy.deepcopy(INITIAL_SPOTS)

def _get_user_location():
    """Mengambil koordinat GPS dari Parameter URL yang di-inject oleh JS"""
...
def _clear_user_location():
    """Menghapus GPS dan kembali ke acuan titik wisata"""
...
# ==================================================================
# 4. FUNGSI LOGIKA (PETA & NAVIGASI OSRM)
# ==================================================================
...
def amenity_card(title: str, items: list, kind: str):
    ...
    for idx, item in enumerate(items):
        ...
        rows += f'<div class="ritam-item"><span>{name}</span><span class="ritam-dist-pill {kind}">{dist}</span></div>'
    st.markdown(f"""
...
def parse_directions(steps):
    ...
    for step in steps:
        ...
        m_mod = step.get('maneuver', {}).get('modifier', 'lurus')
...
def render_map_and_directions(start_coords, end_coords, safe_name, is_live_location=False):
    ...
    # MENGGUNAKAN PETA OPENSTREETMAP (WARNA PUTIH)
    ...
    except Exception: html_inst = "<p style='color:#e6572a; font-size:12px;'>Gagal mengambil rute dari server OSRM.</p>"
    
    m.fit_bounds([start_coords, end_coords])
    return m._repr_html_(), html_inst

# ==================================================================
# 5. STYLE CSS GLOBAL & ADMIN
# ==================================================================
st.markdown("""
<style>
...
with st.container(key="ritam_topbar"):
    ...
    with top_r:
        ...
        if st.session_state.is_admin:
            ...
            if st.button("🔒 Admin"): st.session_state.show_login = not st.session_state.show_login
...
# ==================================================================
# 7. VIEW 1: GRID & TAMBAH DESTINASI (ADMIN)
# ==================================================================
if st.session_state.view == "grid":
    ...
    # Hapus parameter URL spot dan view
            ...
            with st.form("add_spot_form"):
                ...
                if st.form_submit_button("Simpan Destinasi"):
                    st.session_state.spots.append({
                        ...
                        "evac": {"point": n_safe, "bearing": 0, "dist": "-", "time": "-", "note": "Panduan belum ada."},
...
# ==================================================================
# 8. VIEW 2: DETAIL EVAKUASI & LIVE LOCATION GPS
# ==================================================================
else:
    ...
    with tabs[0]:
        ...
        st.markdown(f'<div class="ritam-eyebrow">{"Arah Evakuasi dari GPS Anda" if is_live else "Arah Evakuasi (Titik Acuan)"}</div>', unsafe_allow_html=True)
        ...
        st.markdown(f'<div class="ritam-meta-row"><div>Arah<b>{dir_label} ({dynamic_bearing}°)</b></div></div><div class="ritam-note">{spot["evac"]["note"]}</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
            ...
            if st.session_state.trigger_locate:
                ...
                if (navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(function(position) {
                        ...
                        alert("Akses lokasi ditolak. Pastikan GPS menyala.");
    ...
    with tabs[1]:
        steps = [
            ...
            ("Menuju titik kumpul", "Gunakan jalur jalan kaki (GPS) menghindari macet kendaraan."),
        ...
        with tabs[3]:
            ...
            with st.form(f"add_amenity_{spot['id']}"):
                ...
                if st.form_submit_button("➕ Tambah"):
                    if a_name:
                        spot["amenities"][a_type].append((a_name, a_dist))
